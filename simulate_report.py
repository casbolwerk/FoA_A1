...
def simulate_game(initial_board: SudokuBoard, player1: SudokuAI, player2: SudokuAI, solve_sudoku_path: str, file_to: str, calculation_time: float = 0.5) -> None:
    """
    ...
    with multiprocessing.Manager() as manager:
        ...
        while move_number < number_of_moves:
            ...
            open(file_to, 'a').write(f'Best move: {best_move}' + "\n")
            player_score = 0
            if best_move != Move(0, 0, 0):
                if TabooMove(i, j, value) in game_state.taboo_moves:
                    ...
                    open(file_to, 'a').write(f'Error: {best_move} is a taboo move. Player {2-player_number} wins the game.' + "\n")
                    return
                ...
                if 'Invalid move' in output:
                    ...
                    open(file_to, 'a').write(f'Error: {best_move} is not a valid move. Player {3-player_number} wins the game.' + "\n")
                    return
                if 'Illegal move' in output:
                    print(f'Error: {best_move} is not a legal move. Player {3-player_number} wins the game.')
                    open(file_to, 'a').write(f'Error: {best_move} is not a legal move. Player {3-player_number} wins the game.' + "\n")
                    return
                if 'has no solution' in output:
                    print(f'The sudoku has no solution after the move {best_move}.')
                    open(file_to, 'a').write(f'The sudoku has no solution after the move {best_move}.' + "\n")
                    player_score = 0
...
def main():
    ...
    # # repeat the first test for every non-empty board position
